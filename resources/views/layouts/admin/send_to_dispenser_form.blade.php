...
@section('content')
    <div class="container container-fluid">
        <div class="card" style="background-color: #FCFCFC; width: 45rem;">
            ...
            <form action="{{ route('send_product_to_dispenser') }}" method="POST">
                @csrf
                <input type="hidden" name="product_id" value="{{ $product->Id }}" >
                <div class="card-body">
                    <div class="card-title">
                        <h2>{{ $product->Name }}</h2>
                        <h5 style="white-space: pre-wrap">{{ $product->Details }}</h5>
                    </div>
                        ...
                        <tbody>
                            @foreach ($slots as $slot)
                                <tr>
                                    <td>{{ $slot['Slot'] ?? '' }}</td>
                                    <td>
                                        <input type="checkbox" class="input-lg" name="update_slots[]" value="{{ $slot['Slot'] }}"  > 
                                    </td>
